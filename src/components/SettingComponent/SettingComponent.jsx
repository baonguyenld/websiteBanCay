...
const SettingComponent = ({choice}) => {
  ...
  useEffect(() => {
    const fetchInfoAccount = async () => {
      try {
        const response = await fetch("https://backend-caycanh.onrender.com/api/v1/account/user/"+name);
    ...
    const fetchList = async () => {
      try {
        const response = await fetch("https://backend-caycanh.onrender.com/api/v1/order/searchByUsername/"+localStorage.getItem("username"));
        const jsonData = await response.json();
...
async function changeInfo(infoAccount,type){
  ...
  }else
  {
    if (type==0) {
      ...
      try {
        const response = await fetch("https://backend-caycanh.onrender.com/api/v1/account//user/updateProfile/"+infoAccount["userName"], {
          method: 'PUT',
          headers: {
  ...
  }else
  {
    if (type==0) {
      ...
      if (newElement.value!==renewElement.value) {
        ...
        try {
          const response = await fetch("https://backend-caycanh.onrender.com/api/v1/account/changePassword", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
