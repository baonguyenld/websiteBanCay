...
const PayPage = ({setFlag}) => {
    ...
    useEffect(() => {
        const fetchTotal = async () => {
          try {
            const response = await fetch("https://backend-caycanh.onrender.com/api/v1/plant/all");
            ...
            if (captchaElement.value.toLowerCase() === reCaptchaElement.value.toLowerCase()) {
                if(infoElement.value==""||nameElement.value==""||phoneElement.value==""||addressElement.value=="")
                {
                    ...
                    if(payMethodElement.value=="")
                    {
                        ...
                        try {
                            const response = await fetch("https://backend-caycanh.onrender.com/api/v1/order/createOrder", {
                                method: 'POST',
                                headers: {
  ...
  return (
    <div className="container pay-body">
        ...
        {renderPay(list,name,listTotal,listDetail,payment,captcha,setCaptcha,id,setPrice)}
            <form id="paymentForm" action="https://backend-caycanh.onrender.com/api/v1/payment/create_payment_url" method="POST">
